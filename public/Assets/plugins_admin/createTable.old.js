    ...
    */
		...
		var all = $('.allFilterNew'),
			topPanel = '<form class="mR5" name="frmSearch_'+arDat.elmTable+'" id="frmSearch_'+arDat.elmTable+'" onsubmit="doSearchTable(\''+arDat.url+'\', \''+arDat.elmTable+'\', '+arDat.loadAfter+', '+arDat.showPaging+', '+arDat.id+'); return false;">'+
                ...
                '<div class="form-group mT20 floatBottom">'+
                    '<button type="submit" class="filterBtn btn btn-sm btn-success"><i class="glyphicon glyphicon-search"></i> Filter</button>'+
                    '<button type="reset" class="btn btn-default mL5 reset"><i class="glyphicon glyphicon-erase"></i> Reset</button>'+
                    '<span class="btn btn-default cancelForm mL5 closeFormFloat"><i class="glyphicon glyphicon-ban-circle"></i> Cancel</span>'+
                '</div>'+
            '</form>';

		var bottomPanel = '<div class="pagingWraps"><div class="pull-right">'+
            '<ul class="pagination pull-left" id="pagination_'+arDat.elmTable+'"></ul>'+
            ...
            '</div><span class="pageInfo pull-left mR10" id="pageinfo_'+arDat.elmTable+'"></span>|'+
	...
	function buildTable(elmTable, getData) {
        ...
        if(getData.data != '') {
            $.each(getData.header, function (key, val) {
                if (key != 'action' && key != 'txtName' && key != "txtId" && key != "txtIdDetail" && key != "txtIdHeader") {
                    rowHead.append("<li class='"+getData.class[key]+" "+key+"' rel='"+key+"'><span class='sorter "+key+"' title='Sort this field' rel='"+key+"'></span><i class='glyphicon glyphicon-lock pinBtn' title='Pin this field'></i><span class='lbls'>" + val + "</span><span class='tableResize'></span></li>");
                }
                if (key == 'txtName') {
                    rowHead.append("<li class='lName "+key+"' rel='"+key+"'><span class='sorter "+key+"' title='Sort this field' rel='"+key+"'></span><i class='glyphicon glyphicon-lock pinBtn' title='Pin this field'></i><span class='lbls'>" + val + "</span><span class='tableResize'></span></li>");
                }
                if (key == 'action') {
                    rowHead.append("<li class='"+key+"' rel='"+key+"'><i class='glyphicon glyphicon-lock pinBtn' title='Pin this field'></i><span class='lbls'>" + val + "</span></li>");
                }
            });
        }else{
            $.each(getData.header, function (key, val) {
        ...
        if(getData.data != ''){
            ...
            $.each(getData.data, function(kData, vData){
                var rowList = $("<li class='listTable'></li>"),
                    list = $("<ul class='list-unstyled horz'></ul>");

                $.each(getData.header, function(k, v) {
                    var viewData = vData[k];
                    ...
                    if(k != 'action' && k != 'txtName' && k != "txtId" && k != "txtIdDetail" && k != "txtIdHeader") {
                    ...
                    if(k == 'action') {
                        list.append("<li class='"+k+"'>"+viewData+"</li>");
                    }
                });
                list.append(clear);
                rowList.append(list);
            ...
            var rowList = $("<li class='listTable'></li>"),
                ...
                list.append("<li><i>- - - - Data not found - - - -</i></li>");
                list.append(clear);
                rowList.append(list);
                rowData.append(rowList);

            $('.tables').addClass('emptyOn');
            $('.pagingWraps, .assignOption').addClass('hideOn');
        }
		...
		$('.pinBtn').bind('click', function(){
			...
			if(typeof localStorage.pinned != 'undefined'){
				...
				$(this).parents('.tables').find('.tblHead').css({paddingLeft: mw + pw});
	...
	function tableResize(elm) {
		...
		var wCtn = 0;
		for (var i = 0; i < n; i++) {
			wCtn = wCtn + $('li:eq(' + i + ')', $('.tblContent>li', table)).outerWidth() + 10;
			//console.log($('li:eq(' + i + ')'));
		}
